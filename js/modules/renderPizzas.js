...
const createCard = (data) => {
    ...
    <img class="card__image" src="${data.images[0]}" alt="${data.name.ru}"></picture>
  ...
  export const renderPizzas = async (toppings) => {
    const pizzas = await getData(`https://confused-vanilla-guide.glitch.me/api/products${toppings ? `?toppings=${toppings}` : ''}`,);
